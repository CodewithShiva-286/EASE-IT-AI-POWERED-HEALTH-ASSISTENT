...
exports.generateChatResponse = async ({ userId, prompt, chatHistory, lastScanResult }) => {
    try {
        // Fetch the user's health data from MongoDB
        const user = await User.findById(userId).select('healthConditions');
        const healthConditions = user?.healthConditions?.join(", ") || "No health issues provided";
